...
import { runCase, runApi } from '@/api/run'
...
export default {
  ...
  methods: {
    ...
    parseValidateToRun(data) {
      ...
      return String(JSON.stringify(validate))
    },
    // 类型转换
    ...
    runCase() {
      ...
      for (const step of table) {
        step['validate'] = this.parseValidateToRun(JSON.parse(JSON.stringify(step['validate'])))
      }
      delete table['id']
    ...
    handleValidate(validate) {
      this.$set(this.tableList[this.currentIndexForTable], 'validate', validate)
    },
